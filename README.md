...
이 API Key는 이후 가이드의 **'5단계: SDK 초기화'** 과정에서 `Elegaiter.init()` 함수를 호출할 때 사용됩니다. 발급받은 키를 안전하게 보관해 주세요!
...
> **🚨 주의:** 반드시 5단계의 `Elegaiter.init()`이 성공(SUCCESS)한 이후에 `getInstance()`를 호출해야 합니다. 초기화 전에 호출할 경우 에러가 발생할 수 있습니다.
